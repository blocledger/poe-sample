...
const myApp = angular.module('myApp', ['ngAnimate', 'ngRoute',
...
(function() {
    ...
    myApp.controller('adminCtrl', ['$scope', '$http',
        function($scope, $http) {
            ...
            $http.get(baseUrl + '/chaincode').then(function(response) {
                console.log(response.data);
                let poeChaincode = response.data;
                if (poeChaincode && poeChaincode.chaincodeId && poeChaincode.chaincodeVersion) {
                    $scope.chaincodeName = poeChaincode.chaincodeId;
                    $scope.chaincodeVersion = poeChaincode.chaincodeVersion;
                }
            });

            $scope.saveChaincode = function() {
                $scope.showAlert = false;
                $scope.alertMsg = '';
                $scope.showErrorAlert = false;
                $scope.alertErrorMsg = '';
                $scope.showInfo = true;
                $scope.infoMsg = 'Saving new chaincode configuration';

                var params = {
                    'chaincodeId': $scope.chaincodeName,
                    'chaincodeVersion': $scope.chaincodeVersion,
                };

                $http.post(baseUrl + '/chaincode', params).then(function(response) {
                    ...
                    $scope.alertErrorMsg = error.data;
                });
            };
        ...
        function($scope, $http) {
            $scope.addUserFn = function() {
                var params = {
                    ...
                    'registrarId': $scope.registrarId,
                    'registrarPw': $scope.registrarPw
                };
                ...
                modalInstance.result.then(function() {  //Do this if the user selects the OK button
                    ...
                    $http.post(baseUrl + '/delUser', params)
                        .then(function(response) {
                            ...
                            // We removed the current logged-in user, redirect and refresh to the home page,
                            // so it's apparent that the user needs to log in again.
